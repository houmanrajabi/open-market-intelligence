...
# --- CONFIGURATION ---
API_URL = config.processing.vlm_api_url
MODEL_NAME = config.processing.vlm_model
API_KEY = config.processing.vlm_api_key
...
def encode_image(image_obj, max_size=None):
    """Resize and encode image with quality preservation."""
    
    if max_size is None:
        max_size = config.processing.image_max_size
...
def call_vlm_with_retry(
    ...
    for attempt in range(config.processing.vlm_max_retries):
        try:
            ...
            response = client.chat.completions.create(
                ...
                max_tokens=config.processing.vlm_max_tokens,
                temperature=min(current_temp, config.processing.vlm_temperature * 2)
            )
